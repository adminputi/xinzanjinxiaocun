...
$stmt = $pdo->prepare("SELECT pr.*, s.name as supplier_name, w.name as warehouse_name FROM purchase_returns pr LEFT JOIN suppliers s ON pr.supplier_id=s.id LEFT JOIN warehouses w ON pr.warehouse_id=w.id WHERE pr.id=?");
$stmt->execute([$id]);
$order = $stmt->fetch();
if (!$order) die('退货单不存在');
// 非admin用户只能查看自己的记录
...
$stmt2 = $pdo->prepare("SELECT i.*, p.name as product_name, p.sku, p.spec, u.name as unit_name FROM purchase_return_items i JOIN products p ON i.product_id=p.id LEFT JOIN units u ON p.unit_id=u.id WHERE i.return_id=?");
$stmt2->execute([$id]);
...
// 处理操作
    ...
    if ($postAction === 'confirm' && $order['status'] === 'draft') {
        ...
        try {
            $pdo->prepare("UPDATE purchase_returns SET status='confirmed', created_at=? WHERE id=?")->execute([date('Y-m-d H:i:s'), $id]);
            foreach ($items as $it) {
                update_inventory($it['product_id'], $order['warehouse_id'], -$it['quantity'], 'out', $order['bill_no'], 'purchase_return', get_user_id());
            }
            add_log(get_user_id(), 'confirm', 'purchase_return', "确认退货: {$order['bill_no']}");
            $pdo->commit();
            redirect("return_view.php?id=$id");
        } catch (Exception $e) { $pdo->rollBack(); error_log('Purchase return view confirm error: ' . $e->getMessage()); $error = '确认失败，请查看系统日志'; }
    } elseif ($postAction === 'withdraw' && $order['status'] === 'confirmed') {
        $pdo->beginTransaction();
        try {
            $pdo->prepare("UPDATE purchase_returns SET status='draft' WHERE id=?")->execute([$id]);
            foreach ($items as $it) {
                update_inventory($it['product_id'], $order['warehouse_id'], $it['quantity'], 'in', $order['bill_no'], 'purchase_return_cancel', get_user_id(), '撤回退货');
            }
            add_log(get_user_id(), 'withdraw', 'purchase_return', "撤回退货: {$order['bill_no']}");
            $pdo->commit();
            redirect("return_view.php?id=$id");
        } catch (Exception $e) { $pdo->rollBack(); error_log('Purchase return view withdraw error: ' . $e->getMessage()); $error = '撤回失败，请查看系统日志'; }
    }
}
?>

<div class="page-header">
    <h1 class="page-title"><i class="fa-solid fa-eye"></i> 采购退货单详情</h1>
    <div class="page-actions">
        <?php if ($order['status'] === 'draft'): ?>
        <form method="post" style="display:inline" onsubmit="return confirm('确认退货后库存将减少，确定？')">
            <?= csrf_field() ?>
...
<div class="card">
    <div class="card-header">
        <h3 class="card-title">采购退货单 #<?= $order['bill_no'] ?></h3>
