...
if ($id > 0) {
    ...
    $stmt = $pdo->prepare("SELECT i.*, p.name as product_name, p.sku, p.spec, p.unit_id, u.name as unit_name FROM sales_order_items i JOIN products p ON i.product_id=p.id LEFT JOIN units u ON p.unit_id=u.id WHERE i.order_id=?");
    $stmt->execute([$id]);
    $items = $stmt->fetchAll();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if (empty($itemsData)) { $error = '请至少添加一个商品'; $pdo->rollBack(); }
            ...
            if ($oid > 0) {
                ...
                $pdo->prepare("UPDATE sales_orders SET customer_id=?,warehouse_id=?,total_amount=?,order_date=?,delivery_date=?,employee_id=?,remark=? WHERE id=?")
                    ->execute([$customerId,$warehouseId,$totalAmount,$orderDate,$deliveryDate?:null,$employeeId,$remark,$oid]);
                $pdo->prepare("DELETE FROM sales_order_items WHERE order_id=?")->execute([$oid]);
                add_log(get_user_id(), 'update', 'sales_order', "编辑销售订单: $billNo");
            } else {
                $billNo = generate_bill_no('XS');
                $pdo->prepare("INSERT INTO sales_orders (bill_no,customer_id,warehouse_id,total_amount,order_date,delivery_date,employee_id,remark,user_id,created_at) VALUES (?,?,?,?,?,?,?,?,?,?)")
                    ->execute([$billNo,$customerId,$warehouseId,$totalAmount,$orderDate,$deliveryDate?:null,$employeeId,$remark,get_user_id(),date('Y-m-d H:i:s')]);
                $oid = $pdo->lastInsertId();
                add_log(get_user_id(), 'create', 'sales_order', "新建销售订单: $billNo");
            }
    ...
    } catch (Exception $e) { $pdo->rollBack(); error_log('Order save error: '.$e->getMessage()); $error = '保存失败，请稍后重试'; }
}
...
<div class="page-header">
    <h1 class="page-title"><i class="fa-solid fa-<?= $id>0?'pen-to-square':'plus' ?>"></i> <?= $id>0?'编辑':'新增' ?>销售订单</h1>
    ...
    <form method="post" id="orderForm">
        ...
        <div class="card-body">
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    <label class="form-label">预计交货</label>
